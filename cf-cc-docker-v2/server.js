...
/* Model dependencies */
var vocabulary              = require('./model/vocabulary.json');
var thesaurus               = require('./model/thesaurus.json');
var s_dictionary            = require('./model/s_dictionary.json');
var ignoreList              = require('./model/ignore_list.json');
var operationRulesList      = require('./model/operational_rule_replacements.json');
var blackList               = require('./model/blacklist.json');
var posList                 = require('./model/pos_tags.json');
var nonLiteralsList         = require('./model/non_literals.json');
var transitionsList         = require('./model/transitions.json');
...
// var AlchemyAPI              = require('./watson/alchemyapi');
...
/*
 Note if db connections become an issue, create a persistent connection
 that will be reused, as in the below. Just reconnect it, if it gets disconnected
        ...
        if(req.session.OAUTH_STATE == null){
            ...
            pool.getConnection(function(err, connection) {
                ...
                connection.query('SELECT DISTINCT DEMO_OAUTH_STATE FROM usr_api_keys WHERE DEMO_OAUTH_STATE = ?', [req.session.OAUTH_STATE.toString()], function(err, results) {
                    ...
                    if (err) {
                        console.log('Error while performing confirm login for DEMO_OAUTH_STATE = ' + req.session.OAUTH_STATE.toString() + ' : ' + err);
                      res.sendfile('./public/index.html');
                    }
                        ...
                        console.log('User logged in with DEMO_OAUTH_STATE = ' + req.session.OAUTH_STATE.toString());
...
function doAPIPost(action, req, res){
    ...
    return pool.getConnection(function(err, connection) {
        ...
        connection.query(C.API_KEY_QUERY, [req.body.id.toLowerCase(),req.body.apikey], function(err, results) {
            connection.release(); // always put connection back in pool after last query
            if (err) {
                console.log('Error while performing authentication for ID = ' + req.body.id + ' : ' + err);
                ...
                console.log(action + ' The authenticated user key is: ', results[0].Email);
                action(req,res);
            }
        }); // end connection
    }); // end pool
                ...
                oauth.getUserInfo(token, function(userInfo) {
                    ...
                    pool.getConnection(function(err, connection) {
                        ...
                        connection.query('UPDATE usr_api_keys SET DEMO_OAUTH_STATE = ? WHERE Email = ?', [req.session.OAUTH_STATE.toString(),userInfo.email], function(err, results) {
                            connection.release(); // always put connection back in pool after last query
                            if (err) {
                                console.log('Error while updating DEMO_OAUTH_STATE for ID = ' + userInfo.email + ' : ' + err);
                                ...
                                console.log('oauth.getAccessToken() The authenticated demo user is: ', userInfo.email);

                                req.session.USER_ID = userInfo.user_id;
				req.session.EMAIL= userInfo.email;
                                var domain = userInfo.email;
                                console.log("processing login by usersentry: "+userInfo.email);
                                    ...
                                    //
                                     pool.getConnection(function(err, connection) {
                                        ...
                                        // INSERT IGNORE - If new record, inserts, else ignores the insert operation on duplicate key (Email) found         
                                         connection.query('INSERT IGNORE INTO USR_API_KEYS SET Name=?,Email=?,API_Key=?,DEMO_OAUTH_STATE=?', 
                                        ['',userInfo.email,generatedAPIKey,req.session.OAUTH_STATE.toString()], function(err, results) {       
                                            connection.release(); // always put connection back in pool after last query
                                            if (err) {
                                                console.log('Error while inserting into USR_API_KEYS' + err);
                                            }
                                            else if(results.affectedRows == 0){ // User already exists.
                                                console.log('Returning User identified with ID: ' + req.session.EMAIL);           
                                            }
                                            else {
                                                initAppUsersMap();
 						res.redirect('https://contentclarifier.mybluemix.net/app');
                                                console.log('New User created with ID = ' + req.session.EMAIL);
                                            }
                                        }); // end connection
                                    }); // end pool 
                                }
...
app.get('/user-credentials', function (req, res, next) {
        ...
        console.log("User email is :"+ req.session.EMAIL.toLowerCase().trim() );
	    console.log("User OAUTH State is  :"+ req.session.OAUTH_STATE.toString());
        ...
        pool.getConnection(function(err, connection) {
            ...
            connection.query('SELECT API_Key FROM usr_api_keys WHERE Email=? AND DEMO_OAUTH_STATE=?', [req.session.EMAIL.toLowerCase().trim(),req.session.OAUTH_STATE.toString()], function(err, results) {
                connection.release(); // always put connection back in pool after last query
                if (err) {
                    console.log('Database Error while querying credentials: ' + err);
                    responseObj = { status: "error" };
                    json = JSON.stringify(responseObj);
                    res.end(json);
                    return;
                    ...
                    console.log('No User credentials found for User: ' + req.session.EMAIL);                    
                    ...
                    console.log('User credentials returned for User: ' + req.session.EMAIL);
                    console.log("Credentials found " + results[0].API_Key);
                    console.log("Credentials found2 " + results[0]['API_Key']);
                    responseObj = { status: "success", id: req.session.EMAIL.toLowerCase().trim(), key: results[0].API_Key };
                    json = JSON.stringify(responseObj);
                    res.end(json);
                    return;
    ...
    if(req.get('host').includes("contentclarifier.mybluemix.net")){
        ...
        pool.getConnection(function(err, connection) {           
            ...
            connection.query('UPDATE usr_api_keys SET DEMO_OAUTH_STATE = null WHERE DEMO_OAUTH_STATE = ?', [req.session.OAUTH_STATE.toString()], function(err, results) {
                connection.release(); // always put connection back in pool after last query
                if (err) {
                    console.log('Error while performing logging out for DEMO_OAUTH_STATE = ' + req.session.OAUTH_STATE.toString() + ' : ' + err);
		    res.redirect('https://contentclarifier.mybluemix.net/app');
                   //res.sendfile('./public/Editor.html');
                }
                else {
                    console.log('Logging out user with DEMO_OAUTH_STATE = ' + req.session.OAUTH_STATE.toString());
...
function checkCalculateReadingLevelsReturnResponseObj(calculateReadingLevels, simplifiedText){
    var responseObj;
    if(calculateReadingLevels == C.RETURN_READING_LEVELS){
        ...
        console.log(getLoggingTimestamp() +' Calculation of reading levels complete.');
    }
    else{
        responseObj = { status: "OK",
            usage: C.TERM_OF_USE_MESSAGE,
...
function setCondenseMode(req){
    ...
    console.log("Set condenseMode = " + condenseMode);
    return condenseMode;
}
...
function processURLAndCallAPI(apiFunction, req, res){
    // Fix Issue #36
    var uriEncodedURL = "";
    if(isEncoded(req.body.url) === false)
        uriEncodedURL=encodeURI(req.body.url);
    else
        uriEncodedURL=req.body.url;

    console.log(apiFunction + " : uriEncodedURL = " + uriEncodedURL);

    var parameters = {"features": {"concepts": {}}, "return_analyzed_text": true};
    naturalLanguageUnderstanding.doRequest("url", uriEncodedURL, parameters, function(response) {
        var text = "";
        if(response.body != null && response.body.analyzed_text != undefined){
            text = response.body.analyzed_text;
        }

        // Get rid of a lot of uncessary whitespace
        var tokenizer = new Tokenizer('Chuck');
        tokenizer.setEntry(text);
        text = tokenizer.getSentences().join(" ");

        console.log(text);
        //if(C.DEBUG_VERBOSE_LOGGING) console.log(req.body.data);

        return apiFunction(text, req, res);
    ...
    naturalLanguageUnderstanding.doRequest("text", text, parameters, function(response) {
        ...
        stanfordSimpleNLP.process(modText, function(err, result) {
            ...
            var brillTagger = new Tagger(lexicon_file, rules_file, default_category, function(bError) {
                ...
                console.log(getLoggingTimestamp() + " [apiContextualSimplify] successfully queried for parts of speech. " +  wordsPOS.length);
                ...
                if(enhanceContentMode === 0 || enhanceContentMode === undefined){
                    ...
                    var responseObj = checkCalculateReadingLevelsReturnResponseObj(calculateReadingLevels, simplifiedText);
                ...
                */
                if(enhanceContentMode === C.ENHANCE_DEFINITIONS)
                {                    
                    console.log("enhanceContentMode === " + enhanceContentMode);
                    ...
                    console.log(getLoggingTimestamp() +' Retreiving Definitions Completed');   
                            ...
                            if(error !== undefined && error !== null || results === null){
                                ...
                                for(var noun in properNounsMap){
                                    ...
                                    if( queryMap[dbPediaPageName] !== undefined ){
                                        ...
                                        var thumbnail = queryMap[dbPediaPageName].thumbnail.value;
...
function doAPICondense(text, req, res){
    ...
    console.log("[apiCondense] : text = "+text);
    ...
    if(condenseMode == "extraction"){
        ...
        ClarifierExtractionCondense.summarize("", text, function(err, summary) {
            if(err || summary === undefined || summary === null || summary.length == 0){
                ...
                console.log(contentLengthTxt);
                console.log(summaryLengthTxt);
                console.log(summaryRatioTxt);
        ...
        console.log("Running Abstraction Condense...");
...
var apiCondenseWatsonWorkspaceChat = function(req, res) {
    ...
    if(messages === undefined || messages === null || messages == []){
        console.log("The messages JSON array was not defined.");
        res.writeHead(200, {"Content-Type": "application/json"});
    ...
    try{
        summary = ClarifierChatCondense.getMomentSummary(messages, classifierThreshold, messageFormat);
    }
    catch(e){
        console.log("ClarifierChatCondense module threw an exception.");
        res.writeHead(200, {"Content-Type": "application/json"});
    ...
    if(summary[0].CC_SummaryWordCnt == 0 || 
      (summary[0].CC_SummaryWordCnt == summary[0].ConvoWordCnt))
    { // Handle case where not enough data to summarize.
        console.log("ClarifierChatCondense did not have enough data to process for a summary.");
        res.writeHead(200, {"Content-Type": "application/json"});
...
function doAPICondenseAndSimplify(text, req, res){
    ...
    console.log("[apiCondenseAndSimplify] : text = "+text);
    ...
    if(condenseMode == "extraction"){
        ...
        ClarifierExtractionCondense.summarize("", text, function(err, summary) {
            ...
            if(err || summary === undefined || summary === null || summary.length == 0){
                ...
                console.log('Condense Operation Can not be performed, just attempt to simplify the input.');
        ...
        naturalLanguageUnderstanding.doRequest("text", text, parameters, function(response) {
            ...
            if(naturalLanguageUnderstanding['semantic_roles'].results.length == 0){
                ...
                console.log("Running Condense and Simplify (Lexical)...");
                ...
                var time = end - start;
                console.log('Condense And Simplify Completed in: ' + time + 'ms.');
...
var apiTrain = function(req, res) {
    ...
    console.log(trainingPairs);

    res.writeHead(200, {"Content-Type": "application/json"});

    // var simplifiedText = "SIMPLIFIED! " + text;
    console.log("Running Train Operation...");
    var start = new Date().getTime();
    var resp = trainBayesianNetwork(trainingPairs);
    var end = new Date().getTime();
    var time = end - start;
    console.log('Train Operated Completed in: ' + time + 'ms.');
...
var trainBayesianNetwork = function(trainingPairs) {
    ...
    console.log("Calling trainBayesianNetwork");
...
var contextualLexicalSimplify = function(wordsPOS, enhanceContentMode) {
    ...
    console.log("contextualLexicalSimplify");
    ...
    for (var i = 0; i < wordsPOS.length; i++) {
        ...
        // If we need to enforce operational rules
...
var markProperNounsAndCreateMap = function(text, concepts){
    ...
    console.log("markProperNounsAndCreateMap");
...
var markHTMLFormattingAndCreateMap = function(text){ // Fix #173
    ...
    console.log("markHTMLFormattingAndCreateMap");  
...
var mapWordsToAACSymbols = function(output, wordsPOS){ 
    ...
    console.log("mapWordsToAACSymbols ##############################################");
    ...
    for(var i = 0; i < wordsPOS.length; i++){
            ...
            pos !== "WP$")
        {
            ...
            console.log("Processing word: "+currW);
...
var getSubjectActionObjectRelationMap = function(semantic_roles){
    ...
    console.log("getSubjectActionObjectRelationMap");
    //if(C.DEBUG_VERBOSE_LOGGING) console.log(semantic_roles);
...
var getSimplificationSafeSubjectActionObjectRelationMap = function(text, semantic_roles){
    ...
    try{
        ...
        console.log("getSimplificationSafeSubjectActionObjectRelationMap");
...
function logReadabilityCalculations(x){
    ...
    console.log("Flesch Kincaid (Input) = " + x.fleschKincaidInput);
    console.log("Flesch Kincaid (Output) = " + x.fleschKincaidOutput);
    console.log("Coleman Liau (Input) = " + x.colemanLiauInput);
    console.log("Coleman Liau (Output) = " + x.colemanLiauOutput);
    console.log("Automated Readability Index (Input) = " + x.ariInput);
    console.log("Automated Readability Index (Output) = " + x.ariOutput);
}
...
function calculateFleschKincaid(text){
    ...
    try{
        ...
        console.log("Err in calculateFleschKincaid " + err);
        return -1;
    }
}
...
var markNonLiteralsAndTransitions = function(text){
    ...
    console.log("markNonLiteralsAndTransitions");
...
function initAppUsersMap(){
    ...
    console.log(getLoggingTimestamp() +' Poll for demo users');

    pool.getConnection(function(err, connection) {
        if(err) {
            console.log('DB Error during initAppUsersMap(): ' + err);
        }
        connection.query(C.DEMO_USERS_QUERY, [], function(err, results) {
            connection.release(); // always put connection back in pool after last query
            if (err) {
                console.log('Error while querying for demo users : ' + err);
            }
            else if(results[0] === undefined){ // Didn't find any demo users
                console.log('No demo users found in the DB. ');
            }
            else {
                var i = null;
...
function initNonLiteralsMap() {
    ...
    console.log("initNonLiteralsMap");
    ...
    for (i = 0; nonLiteralsList.length > i; i += 1) {
        ...
        if (key != undefined) {
            ...
            console.log("REPLACEMENT= " + nonLiteralsList[i].REPLACEMENT);
...
function meterAPICalls(apiCall, options, bodyLength, id, apiKey){
    ...
    pool.getConnection(function(err, connection) {
        if(err) {
            console.log('DB Error in meterAPICalls(): ' + err);
        }
        connection.query(C.API_CALLS_INSERT_QUERY, {Email: id,
            ...
            connection.release(); // always put connection back in pool after last query
            if (err) {
                console.log('Error while inserting API call: ' + err);
            }
        }); // end connection
    });
    ...
    for(var i=0; i<bluemixContainers.length; i++){
        ...
        objStorage.getContainer(containerName)
            ...
            .then(function(objectList) {
                ...
                console.log("Processing " + objectList.length + " objects in container");
                ...
                for(var j=0; j<objectList.length; j++)
                {
                    ...
                    if(currContainer === undefined){
                        console.log("ERROR currContainer is UNDEFINED!");
                        return;
                    }
                    ...
                    dbConn.query(C.AAC_SYMBOLS_INSERT_QUERY,
                        ...
                        function(err, result) {
                            if (err) {
                                console.log('Error while inserting aac symbol row: ' + err);
                            }
                        }); // end connection.query

                } // end for

                console.log("Completed processing for " + containerName);
            })
            .catch(function(err) {
                console.log(err);
            });
...
function saveAACSymbolsFullTextSearch(){
    ...
    console.log(getLoggingTimestamp() +' saveAACSymbolsFullTextSearch()');

    pool.getConnection(function(err, connection) {
        if(err) {
            console.log('DB Error during saveAACSymbolsFullTextSearch(): ' + err);
        }
        connection.query(C.AAC_SYMBOLS_QUERY, [], function(err, results) {
            connection.release(); // always put connection back in pool after last query
            if (err) {
                console.log('Error while querying for aac symbols : ' + err);
            }
            else if(results[0] === undefined){ // Didn't find any demo users
                console.log('No aac symbols found in the DB. ');
            }
            else {
                try{
                    ...
                    for (i = 0; results.length > i; i += 1) {
                        ...
                        search.add(obj);
                        console.log("added " + results[i].name + " : i = " + i);
                    }
                    console.log("Add " + i + " objects to search");

                    // Save symbols to JSON file
                    search.saveSync('./model/AACSymbols.json');

                    console.log("Completed saveSync");
...
function initAACSymbolsMap(){
    ...
    pool.getConnection(function(err, connection) {
        if(err) {
            console.log('DB Error during initAACSymbolsMap(): ' + err);
        }
        connection.query(C.AAC_SYMBOLS_QUERY, [], function(err, results) {
            connection.release(); // always put connection back in pool after last query
            if (err) {
...
var server = app.listen(port,function(){
    ...
    console.log("Initing Lookup Maps...");
    var start = new Date().getTime();
    ...
    //ClarifierChatCondense.getMomentSummary(testJSON, 0.85, "short"); // WW DEBUG
    //return;
...
server.timeout = 300000; // 5 min server timeout allow time for longer requests
