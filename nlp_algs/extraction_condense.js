// Implements Naive Summarization Algorithm described here - https://thetokenizer.com/2013/04/28/build-your-own-summary-tool/

//var _ = require('lodash-node/underscore'); // Original line
...
function formatSentence(sentence, callback) {
	...
	//callback(sentence.replace(re, '')); // WSCOTT - broken, just return sentence for now.
	callback(sentence);
}
...
exports.summarize = function(title, content, callback) {
	...
	getSentencesRanks(content, function(dict) {
		splitContentToParagraphs(content, function(paragraphs) {
			...
			callback(err, summary.join(" ")); // WSCOTT
		});
	});
};

exports.getSortedSentences = function(content, n, callback) {
