/**
   ...
   Check remaining API quota: curl -i http://access.alchemyapi.com/calls/info/GetAPIKeyInfo?apikey=<apiKey>
*/
...
function AlchemyAPI() {
	...
	//Load the key from alchemyapi.config file
	try {
        //console.log(__dirname);
		key = fs.readFileSync(__dirname + '/alchemyapi.config').toString().trim();
	}
	catch(err) {
		//Create the file
		console.log('API key not detected in alchemyapi.config');
          ...
          //callback(JSON.parse(response)); 
            if (response){
                try{
                    ...
                    console.log("alchemyapi.js onEnd: ");
                    console.log(response);
                    console.log("onEnd exception: ");                    
                    console.log(e);  
                }
            } 
          else{
            console.log("alchemyapi.js Empty response.");              
          }
      });
      res.on('error', function (err) {
        console.log("alchemyapi.js onError: ");
